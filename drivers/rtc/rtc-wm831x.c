...
static int wm831x_rtc_set_mmss(struct device *dev, unsigned long time)
{
	...
	int count = 0;
        ...
        unsigned short rtc_time[3];

        rtc_time[0] = (time >> 16) & 0xffff;
        rtc_time[1] = time & 0xffff;

	ret = wm831x_reg_muti_write(wm831x, 0x4021, rtc_time, 2);

	if (ret < 0) {
		dev_err(dev, "Failed to write RTC_TIME: %d\n", ret);
		return ret;
	}
	...
	if (ret < 0) {
		dev_err(dev, "Failed to readtime: %d\n", ret);
		return ret;
	}
