...
#define BATT_MAX_VOL_VALUE               8466 //   4180               	//满电时的电池电压	 FOR A7
#define BATT_ZERO_VOL_VALUE            6806 //    3500              	//关机时的电池电压
#define BATT_NOMAL_VOL_VALUE             7350 //  3800               
...
static int rk29_adc_battery_load_capacity(void)
{
    ...
    long fd = sys_open(BATT_FILENAME,O_RDONLY,0777);
    if(fd < 0)
    {
    ...
    {
            printk("----------------open file /data/bat_last_capacity.dat ok\n");
    }

     int ret = sys_read(fd,(char __user *)value,4);      	
     sys_close(fd);
...
static void rk29_adc_battery_put_capacity(int loadcapacity)
{
    ...
    if( bat_lock == 1 ){
        printk("!!!!!bat_lock = %d   return!!!!!\n", bat_lock);
        return;
    }

    long fd = sys_open(BATT_FILENAME,O_CREAT | O_RDWR,0777);    
    if(fd < 0)
    {
	  return;
    }
    ...
    *p = loadcapacity;
     ret = sys_write(fd, (const char __user *)value, 4);
     sys_close(fd);
...
static int g_read_file_times = 0;
...
static int rk29_adc_battery_resume(struct platform_device *dev)
{
	...
	queue_work(gBatteryData->wq, &gBatteryData->resume_work);
	return 0;
}
#else
...
static int rk29_adc_battery_io_init(struct rk29_adc_battery_data *data, struct rk29_adc_battery_platform_data *pdata)
{
	...
	if (pdata->charge_set_pin != INVALID_GPIO)
	{
    	...
    	if (ret) {
    		printk("failed to request charge_set gpio\n");
    		goto error;
    	}
    	gpio_direction_output(pdata->charge_set_pin, 1 - pdata->charge_set_level);
    }
...
#if 1
	data->wq = create_singlethread_workqueue("adc_bat");
...
module_init(rk29_adc_battery_init);
